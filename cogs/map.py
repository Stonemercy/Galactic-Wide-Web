...
class MapCog(commands.Cog):
    ...
    async def update_message(self, i: PartialMessage, embed_dict):
        ...
        try:
            await i.edit(embed=embed_dict[guild[5]])
        except NotFound:
            self.messages.remove(i)
            Guilds.update_map(i.guild.id, 0, 0)
            return logger.error(f"MapCog Map not found, removing, {i.channel.name}")
        except Forbidden:
            self.messages.remove(i)
            Guilds.update_map(i.guild.id, 0, 0)
            return logger.error(f"MapCog Map forbidden, removing, {i.channel.name}")
        except Exception as e:
            return logger.error(f"MapCog Map update_message, {e}, {i.channel.name}")
